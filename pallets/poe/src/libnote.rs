...
pub mod pallet {
	...
	#[pallet::config]
	// add220831，parity官方没有frame_system::Config，只有frame_system::pallet::Config，但因为pallet里使用了pub use pallet::*；Config就直接对frame_system可见了。
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		// add220901,插件显示Get存储在sp_runtime::traits，但实际却是在 .cargo>git>... 里存放，是因为这里用的不是完整的substrate，而是node-template。sp_runtime::traits是被作为
		// 外部依赖从github的substrate处引用来的，所以最终存放在来奇怪的路径。但为什么不直接用u32而用Get<u32>没弄明白mmms。
